    ...
    for (const issue of assignedIssues) {
      ...
      try {
        const linkedPRs = await github.paginate(
          ...
          (response, done) => {
            ...
            return response.data.filter(
              (event) => new Date(event.created_at) >= threeMinutesAgo
            );
          },
          { maxPages: MAX_PAGES }
        );
        ...
        if (lastNonBotActivity < threeMinutesAgo) {
          console.log(
            `Unassigning issue #${issue.number} due to 3 minutes of inactivity`
          );
        ...
        } else if (lastNonBotActivity < twoMinutesAgo && !warningAlreadyGiven) {
          console.log(
            `Warning on issue #${issue.number} due to 2 minutes of inactivity`
          );
          await github.rest.issues.createComment({
            owner: context.repo.owner,
            repo: context.repo.repo,
